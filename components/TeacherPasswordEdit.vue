                  ...
                  <div class="px-4 sm:px-6  md:px-12 flex flex-row justify-between items-center">
                    <div class="flex flex-col">
                      <h2 id="slide-over-heading-2" class="text-xl font-medium text-gray-900 mb-2">
                        {{ user.name + " " + user.lastName }} Şifre Değiştir
                      </h2>
                    </div>
                    <div class="flex">
                    </div>
                            ...
                            <div class="px-4 py-5 bg-white sm:p-6">
                              ...
                              <div class="grid grid-cols-6 gap-6 mb-5">
                                <div class="col-span-6 sm:col-span-3">
                                  <label for="name" class="block text-sm font-medium text-gray-700">Şifre</label>
                                  <input type="password" name="name" id="name" v-model="passswordData.password"
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                  <label for="name3" class="block text-sm font-medium text-gray-700">Şifre Tekrar</label>
                                  <input type="password" name="name" id="name3" v-model="passswordData.repassword"
                                         class="border mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                </div>
                              </div>
                            </div>
...
export default {
  ...
  methods: {
    changeUserAddForm() {
      this.$emit('changeUserPasswordEditForm');
    },
    handleAddForm() {
      this.formErrors = [];
      this.loginLoading = true;
      this.$axios.put(`/teachers/${this.userId}/passwords`, this.passswordData).then(res => {
  ...
  created() {
    this.$axios.get(`/teachers/${this.userId}`).then(res => {
      this.user = res.data;
    }).catch(err => {
