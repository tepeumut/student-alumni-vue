                      ...
                      <div class="md:grid md:grid-cols-3 md:gap-6">
                        <div class="md:col-span-1">
                          <div class="px-4 sm:px-0">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">Anket Soru Güncelleyin</h3>
                          </div>
                        </div>
                              ...
                              <div class="px-4 py-5 bg-white sm:p-6">
                                ...
                                <div class="grid grid-cols-6 gap-6">
                                  <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <input type="text" name="name" id="name" v-model="survey.question"
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                  ...
                                  <select v-model="survey.type" id="type" name="type"
                                    ...
                                    <option :value="category.name" v-for="category in surveyStatusList"
                                            :key="category.id">
                                      {{ category.value }}
                                    </option>
                                  </select>
                                </div>
                                <div v-if="survey.type === 'CHECKBOX' || survey.type === 'RADIO'">
                                  <button type="button"
                                          @click="addQuestionData"
                                  ...
                                  <div class="grid grid-cols-6 gap-6" v-for="(qData, index) in questionData">
                                    <div class="col-span-6 sm:col-span-2">
                                      <label class="block text-sm font-medium text-gray-700">{{ index + 1 }}.
                                        Seçenek Değeri</label>
                                      <input type="text" name="name" v-model="qData.name"
                                             class="border mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    </div>
                                    <div class="col-span-6 sm:col-span-2">
                                      <label class="block text-sm font-medium text-gray-700">{{ index + 1 }}.
                                        Seçenek</label>
                                      <input type="text" name="name" v-model="qData.value"
                                             class="border mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    </div>
                                    <button type="button"
                                            @click="deleteQuestion(index)"
                                            class="col-span-2 inline-flex mt-5 justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                      Seçenek Sil
                                    </button>
                                  </div>
                                </div>
...
export default {
  ...
  data: () => ({
    ...
    formErrors: [],
    surveyStatusList: [
      {id: 1, name: "INPUT", value: "Giriş"},
      {id: 2, name: "TEXT", value: "Metin"},
      {id: 3, name: "QUALITY", value: "Aralık"},
      {id: 4, name: "CHECKBOX", value: "Çoktan Seçmeli"},
      {id: 5, name: "RADIO", value: "Tek"},
    ]
  }),
  methods: {
    changeQuestionEditForm() {
      this.$emit('changeQuestionEditForm');
    },
    addQuestionData() {
    ...
    handleAddForm() {
      ...
      this.survey.surveyId = this.$route.params.id;
      this.survey.data = JSON.stringify(this.questionData);
      this.$axios.put("/surveys/questions/" + this.questionId, this.survey).then(res => {
  ...
  created() {
    this.$axios.get("/surveys/questions/" + this.questionId).then(res => {
      ...
      this.questionData = JSON.parse(this.survey.data);
    }).catch(err => {
