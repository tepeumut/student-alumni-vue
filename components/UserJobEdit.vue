                      ...
                      <div class="md:grid md:grid-cols-3 md:gap-6">
                        <div class="md:col-span-1">
                          <div class="px-4 sm:px-0">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">İş Deneyimi Düzenleyin</h3>
                          </div>
                        </div>
...
export default {
  ...
  methods: {
    changeJobEditForm() {
      this.$emit('changeUserJobEditForm');
    },
    handleEditForm() {
      this.formErrors = [];
      this.loginLoading = true;
      this.$axios.put("/user-jobs/" + this.jobId, this.job).then(res => {
        this.loginLoading = false;
        this.changeJobEditForm();
  ...
  created() {
    this.$axios.get("/user-jobs/"+ this.jobId).then(res => {
      this.job = res.data;
    }).catch(err => {
